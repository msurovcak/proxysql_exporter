...
func scrapeMysqlCommandCounters(db *sql.DB, ch chan<- prometheus.Metric) error {
	...
	for rows.Next() {
		...
		for i, l := range mysqlCommandLabels {
			m := mysqlCommandCountersMetrics["command_count"]

			ch <- prometheus.MustNewConstMetric(
				prometheus.NewDesc(
					prometheus.BuildFQName(namespace, "command", "count"),
					m.help,
		...
		m := mysqlCommandCountersMetrics["command_total_time"]

		ch <- prometheus.MustNewConstMetric(
			prometheus.NewDesc(
				prometheus.BuildFQName(namespace, "command", "total_time"),
				m.help,
				[]string{"command"},
				nil,
			),
